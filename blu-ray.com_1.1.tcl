...
#
# Known issues: 
# - The search result in a few cases is little off, but it's all because
# of how the blu-ray.com search function is made. Works good most of the time.
...
proc bluray_pub {nick uhost hand chan text} {
	...
	set bluray_url "http://www.blu-ray.com/search/?action=search&section=movies&keyword=$bluray_keywords&searchbutton.x=0&searchbutton.y=0"
	...
	if {[regexp {<h3>(.*?)</h3></a>} $html match bluray_title] } {
		...
		if {[regexp {^\-details.*$} [lindex $text 0]]} {
			if {[regexp {td>\s*<td\swidth=\"85\%\">\s*<a\shref=\"(.*?)\"\sclass=\"noline\"><h3>} $html match bluray_detailurl]} {
	        ...
	        if {[regexp {alt=\"United\sKingdom\"\stitle=\"United\sKingdom\"\s\/><br><br>(.*?)<br><br><h5>} $html match bluray_noresult] } {
